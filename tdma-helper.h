...
public:
  ...
  /**
   * This Constructor used just for adding some attributes for the second interface for authentication
   * \param numNodes number of nodes in the TDMA frame
   * \param numSlots total number of slots per frame
   * \param maxRange is used to define the range of the AN node only for authentication
   * \param gRange is used to define the range of the GS node only for authentication
   * \param gID is used to define ID of the GS 
   */
  TdmaHelper (uint32_t numNodes, uint32_t numSlots, 
              double maxRange, double gRange, int gID);
