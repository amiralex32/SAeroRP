...
do
 export NS_GLOBAL_VALUE="RngRun=$trial"
  for attackstatus in $ATTACKSTATUS
   do
    for seed in $SEED
     do
      echo creating $nodes nodes, SEED $seed, ATTACKSTATUS $attackstatus
       export NS_GLOBAL_VALUE="RngSeed=$seed"
    ../../waf --run "aero-test-final --format=db --seed=$seed --nWifis=$nodes --attackstatus=$attackstatus --run=run-$nodes-$seed-$attackstatus"
    done
  done
done
mv ../../data.db .
