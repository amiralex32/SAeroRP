...
def configure(conf):
    ...
    conf.env['lcryptopp'] = conf.check(mandatory=True, lib='cryptopp', uselib_store='CRYPTOPP')
...
def create_ns3_program(bld, name, dependencies=('core',)):
    ...
    if program.env['ENABLE_STATIC_NS3']:
        if sys.platform == 'darwin':
            program.env.STLIB_MARKER = '-Wl,-all_load,-lcryptopp,-lssl'
        else:
            program.env.STLIB_MARKER = '-Wl,--whole-archive,-Bstatic,-lcryptopp,-lssl'
            program.env.SHLIB_MARKER = '-Wl,-Bdynamic,--no-whole-archive,-lcryptopp,-lssl'
        ...
        if program.env.DEST_BINFMT == 'elf':
            ...
            if 'gcc' in (program.env.CXX_NAME, program.env.CC_NAME): 
                program.env.append_value ('SHLIB_MARKER', '-Wl,--no-as-needed,-lcryptopp,-lssl')
