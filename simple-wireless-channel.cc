...
SimpleWirelessChannel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::SimpleWirelessChannel")
    ...
    .AddAttribute ("GroundRange",
                   "Maximum Transmission Range of ground station (meters)",
                   DoubleValue (250),
                   MakeDoubleAccessor (&SimpleWirelessChannel::m_grange),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("GroundID",
                   "ID of ground station",
                   IntegerValue (0),
                   MakeIntegerAccessor (&SimpleWirelessChannel::m_gid),
  ...
  for (TdmaMacLowList::const_iterator i = m_tdmaMacLowList.begin (); i != m_tdmaMacLowList.end (); ++i)
    {
      ...
      if(sender->GetDevice ()->GetNode ()->GetId() == (unsigned int)m_gid)
        range = m_grange;
      ...
      NS_LOG_DEBUG ("Distance: " << distance << " Max Range: " << range);
      if (distance > range)
        {
          continue;
        }
