...
TdmaHelper::TdmaHelper (uint32_t numNodes, uint32_t numSlots, 
                        double maxRange, double gRange, int gID) : m_controller (0),
                                                                m_controllerHelper (0),
                                                                m_slotAllotmentArray (0),
                                                                m_numRows (numNodes),
                                                                m_numCols (numSlots + 1)
{
  ...
  m_channel = CreateObjectWithAttributes<SimpleWirelessChannel> (
        "MaxRange", DoubleValue(maxRange), 
        "GroundRange", DoubleValue(gRange),
        "GroundID", IntegerValue(gID)        
        );
  Allocate2D ();
  SetDefaultSlots ();
}
